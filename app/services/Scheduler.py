...
class SchedulerService:
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            with cls._lock:
                if not cls._instance:
                    cls._instance = super(SchedulerService, cls).__new__(
                        cls, *args, **kwargs
                    )
    ...
    def _schedule_tasks(self):
        ...
        self.scheduler.add_job(process_articles, "cron", hour="*/6")

        self.scheduler.add_job(store_all_recents, "cron", hour="*/12", minute="30")
